...
- Turn rate is **dynamic**: stored in AI data offset 0x0C, updated per state (0x222 for walk, 0x444 for run). `TurnTo` reads this value each frame
...
- **Torso tracking** via `ENTITY_PITCH` (offset 0x68): in run state, torso tilts at half the turn delta (leans into turns). In other states, torso centers back to 0. Clamped ±0x222 per frame
