...
- Diagonal ceilings (types 9-18) use packed corner heights with 4 nibbles per word
- A global flag controls whether steep slopes (gradient magnitude ≥ 3) are skipped entirely
- Entity triggers look up the entity's model type to call a ceiling height callback — these can modify the returned height
...
```javascript
// Get sector first, then calculate ceiling height
...
function CalculateCeilingHeight(sector, x, y, z):
    ...
    if sector has floor data:
        ...
        if diagonal ceiling entry (types 9-18):
