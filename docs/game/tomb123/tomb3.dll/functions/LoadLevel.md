...
- **TR3-specific**: Level-specific save data propagation uses bitmask `0x12220` for levels (1, 5, 9, 13, 16) and range check for levels 22-25
- **TR3-specific**: Level save data structures are 0x40 bytes each
...
function LoadLevel(levelId, mode, param3, assetFile):
    ...
    if (result - 0x400) & 0xFFFFFEFF == 0:  // 0x400 or 0x500
        return result
    ...
    if menuSelection != 0:
        ...
        if loadMode == 1:
            ...
            // Level restart with state copy (TR3-specific ranges)
            if levelId >= 2 and levelId <= 18:
                // Skip if not in bitmask 0x12220 (levels 1,5,9,13,16)
                if (0x12220 >> levelId) & 1:
                    // Copy current stats to level save slot
                    copyCurrentStatsToLevelSlot(levelId)
                    return levelId
            
            if levelId >= 22 and levelId <= 25:
