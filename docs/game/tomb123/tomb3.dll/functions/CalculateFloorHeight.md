...
Also sets two global side-effect variables: a slope type flag (0 = flat, 1 = gentle slope, 2 = steep slope, 3 = diagonal steep, 4 = diagonal) and a current trigger pointer (the first trigger entry found in the floor data, if any).
...
- Slope type is classified as gentle (both gradients abs < 3), steep (either ≥ 3), or diagonal variants
- Diagonal floors (types 7-14) use packed corner heights with 4 nibbles per word
...
```javascript
// Get sector first, then calculate floor height
...
function CalculateFloorHeight(sector, x, y, z):
    ...
    if sector has floor data:
        for each entry:
            ...
            if diagonal floor entry (types 7-14):
                slopeType = 4
                extract 4 corner heights from packed nibbles
                determine which triangle based on X/Z position
                compute gradient from corner heights

                // skip if steep slope flag is active and slope is too steep
                if steepSlopeFlag and (abs(zGrad) >= 3 or abs(xGrad) >= 3):
                    skip
                else if slope is steep:
                    slopeType = 3

                // interpolate height from position within sector
                height += base offset + interpolated gradients

            if trigger entry (type 4):
