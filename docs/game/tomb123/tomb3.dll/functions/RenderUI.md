...
## Notes
- **TR3-specific**: Uses `LaraClimbState == 4` to bypass hide logic (different from TR2's `LaraGunType == 4`)
...
- The bar is hidden when: HP > 250, timer has expired, and climb state is not 4
...
function RenderUI():
    ...
    if hp < 251:  // critical HP
        ...
        if displayTimer < 1 and LaraClimbState != 4:
            return  // hide bar
