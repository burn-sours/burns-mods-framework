...
## Notes
- Returns 1 (can interpolate) immediately if all position deltas are small (< 256 units) and all angle deltas are small (< 8193 units) — the camera barely moved, so interpolation is safe
...
- Also returns 0 if the game is in camera mode 4 or the render flag indicates a cutscene-like state
...
function CanInterpolateCamera():
    ...
    if abs(previousX - currentX) < 256
    and abs(previousY - currentY) < 256
    and abs(previousZ - currentZ) < 256
    and abs(previousPitch - currentPitch) < 8193
    and abs(previousYaw - currentYaw) < 8193
    and abs(previousRoll - currentRoll) < 8193:
        return 1  // small movement, safe to interpolate

    // slow path: geometry validation
    if previous position is not in a valid room:
        return 0
    if game is in camera mode 4 or render flag bit 2 is set:
        return 0
    ...
    if abs(deltaX) < abs(deltaZ):
        resultA = TraceRangeX(current, previous)
        resultB = TraceRangeZ(current, previous)
    else:
        resultA = TraceRangeZ(current, previous)
        resultB = TraceRangeX(current, previous)

    if resultB == 0:
        return 0  // range trace failed

    // update sector for the previous position
    GetSector(previousX, previousY, previousZ, roomId)

    // final line of sight check
    if line of sight clear and resultA == 1 and resultB == 1:
        return 1
