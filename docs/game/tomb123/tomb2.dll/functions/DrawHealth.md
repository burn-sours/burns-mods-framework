...
## Description
Renders Lara's health bar on screen. Takes a health value parameter and draws a styled bar with game-version-specific color schemes. Handles both modern rendering (using DrawSetup, DrawRect, DrawQuad) and OG graphics mode with a fallback rendering path.

## Notes
- Position is calculated relative to UiDrawWidth (right-aligned, offset -108 from right edge)
- Has three different color schemes based on game version:
  - **Version 0**: Blue theme (0xff2c5c70, 0xff4878a4, etc.)
  - **Version 1**: Red-green gradient theme
  - **Version 2**: Green theme (0xff000040/0xff008000, etc.)
- Each version has an OG graphics fallback path
...
game.callFunction(game.module, 'DrawHealth', 100);
```

## Pseudocode
```
function DrawHealth(healthWidth):
